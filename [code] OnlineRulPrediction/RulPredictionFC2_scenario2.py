...
def TFT_model_future_prediction_module(train_data, max_encoder_len_param, max_prediction_len_param, best_tft, batch_size, selected_c_name):
    ...
    for c_name in list(futu_encoder_data.columns):
        if c_name== "Time (h)":
            ...
            futu_decoder_data[c_name]= ["battery2"]* max_prediction_len_param
        else:
...
if __name__== "__main__":
    ...
    eol_H= np.array([eol_H[np.where(np.diff(eol_H)>= 30)[0][2]+ 1]])
    ...
    knee_point= knee_point+ 500
    knee_onset= knee_onset+ 500
    knee_onset_double_bacon_watts= knee_onset_double_bacon_watts+ 500

    selected_c_name= ["J (A/cm2)", "I (A)", "TinWAT"]

    data= data.dropna(axis= 1)
    polarization_est_h= [35, 181, 343, 505, 666, 830]
    data= Interpolation(x= data, removed_index= polarization_est_h)
    original_data= Interpolation(x= original_data, removed_index= polarization_est_h)
    ...
    original_data["Time_series_id"] = "FC2"
    ...
    data["Time_series_id"]= "battery2"
    original_data.iloc[:, 1: -1]= scalar_x.transform(original_data.iloc[:, 1: -1])
    original_data["Time_series_id"] = "battery2"
    ...
    for i in range(0, max_prediction_step):
        ...
        if len(np.where(np.array(model_future_predictions)<= eol_TH)[0]) != 0:
            ...
            print("RE: %2.4f"%(round((np.abs(actual_rul- predicted_rul)/ actual_rul* 100)[0], 4)))
            break
        else:
    ...
    plt.figure(figsize= (6, 3))
    plt.plot(original_data["Time (h)"].iloc[500: unknow_data.index[0]], original_data["Utot (V)"].iloc[500: unknow_data.index[0]], label= "known past data", color= "black")
    ...
    plt.hlines(eol_TH, original_data["Time (h)"].iloc[500], original_data["Time (h)"].iloc[-1], label= "EOL", color= "red", ls= "--")
