...
if __name__== "__main__":
    ...
    selected_c_name= ["DinAIR (l/mn)", "TinWAT", "J (A/cm2)", "I (A)", "ToutAIR"]
    ...
    max_encoder_length= int(len(data)*0.95)
    max_decoder_length= int(len(data)*0.05)
    ...
    for i in range(0, max_prediction_step):
        ...
        if len(np.where(np.array(model_future_predictions)<= eol_TH)[0]) != 0:
            ...
            print("RE: %2.4f"%(np.abs(actual_rul- predicted_rul)/ actual_rul* 100))
            break
        else:
    ...
    plt.scatter(original_data["Time (h)"].iloc[knee_point], original_data["Utot (V)"].iloc[knee_point], label= "initial prediction Sp", color= "darkgreen")
